# Template Style Manager (PowerShell)
# Usage: style <command> <templatePath>
...
if ($Command -eq "--help" -or $Command -eq "-h") {
    Write-Host "Template Style Manager"
    Write-Host ""
    Write-Host "USAGE:"
    Write-Host "  style --list <templatePath>"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "EXAMPLES:"
    Write-Host "  style --list builds\jbc\workspace\jbcNormal"
    exit 0
}

# Get the script directory to find the bash script
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$BashScript = Join-Path $ScriptDir "style"
...
if ($TemplatePath) {
    ...
    & $GitBash -c "bash '$BashScript' '$Command' '$UnixPath'"
} else {
